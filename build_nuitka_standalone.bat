...
REM ScaleLogger - Python 3.12 64-bit standalone build
REM Result: a self-contained 64-bit build folder; Python is not needed on the target machine.
...
echo Standalone build complete. The output folder contains a self-contained 64-bit build.
echo Use generate_sha256.bat on the packaged executable before sharing it.
endlocal
